...
- get_recommended_products  : recommandations personnalisées basées sur l'historique d'achat
- get_categories            : catégories du catalogue
- search_products           : chercher par mot-clé ou catégorie (base locale instantanée)
- get_product_details       : détails complets d'un produit
- verify_product_live       : vérifier prix ET stock EN TEMPS RÉEL via l'API
- compare_products          : comparer 2–3 produits
- get_pending_orders        : commandes en cours (non livrées) pour un client
...
- prepare_order_recap       : récapitulatif avant confirmation (vérifie les prix en direct)
...
━━━ CATALOGUE LOCAL — RÈGLES OBLIGATOIRES ━━━
- TOUJOURS utiliser search_products ou get_recommended_products pour les suggestions produit — JAMAIS improviser un nom ou un prix de mémoire.
- Pour un client identifié qui demande des suggestions → appelle get_recommended_products (résultats basés sur son historique).
- Pour un client anonyme ou sans historique → appelle search_products par catégorie ou mot-clé.
- Disponibilité / prix exact en temps réel → verify_product_live (quand client demande « est-ce en stock ? », « quel est le prix actuel ? »).
- Le prix dans prepare_order_recap est déjà vérifié en direct — ne mentionne pas la vérification à moins que le prix ait changé.
...
_PRODUCT_LIST_TOOLS   = {"search_products", "compare_products", "get_recommended_products"}
_PRODUCT_DETAIL_TOOLS = {"get_product_details", "verify_product_live"}
...
def run_agent(session_id: str, user_message: str) -> dict:
    ...
    for tool_name, result in tool_results_by_name.items():
        if tool_name in ("search_products", "get_recommended_products"):
            ...
            product_ui = result
        elif tool_name == "verify_product_live" and result.get("verified"):
            product_ui = {
                "id":             result["product_id"],
                "name":           result.get("name", ""),
                "price_final":    result["price_live"],
                "price_original": result.get("price_original", result["price_live"]),
                "has_discount":   result.get("has_discount", False),
                "stock":          result["stock_live"],
                "in_stock":       result["in_stock"],
                "price_verified": True,
                "price_changed":  result.get("price_changed", False),
                "price_alert":    result.get("price_message"),
                "stock_warning":  result.get("warning"),
            }
